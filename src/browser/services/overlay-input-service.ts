...
export class OverlayInputService {
  ...
  async assureExclusiveModeWindow() {
    ...
    this.exclusiveModeBackgroundWindow = await this.overlayApi.createWindow(options);
  ...
  onUpdateGameWindow(window: GameWindowInfo) {
    ...
    this.exclusiveModeBackgroundWindow.window.setSize(window.size.width, window.size.height);
  }
  ...
  private onHotKeyAutoRelease(pressed: boolean, inputInfo: GameInputInterception) {
    if (!pressed) {
