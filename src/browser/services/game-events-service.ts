...
export class GameEventsService extends EventEmitter {
  ...
  private registerOverwolfPackageManager() {
    const waitForPackages = () => {
      if (!app.overwolf?.packages) {
        setTimeout(waitForPackages, 100);
        return;
      }
   ...
   *
   * @param {overwolf.packages.OverwolfGameEventPackage} gep The GameEventPackage instance
   */
  private async onGameEventsPackageReady() {
    // Save package into private variable for later access
    if (!app.overwolf?.packages?.gep) {
      this.emit("log", "GEP package not available");
      return;
    }
    ...
    this.gepApi.on("game-detected", (e, gameId, name, gameInfo) => {
      ...
      if (!this.gepGamesId.includes(gameId)) {
        // Stops the GameEventPackage from connecting to the game
